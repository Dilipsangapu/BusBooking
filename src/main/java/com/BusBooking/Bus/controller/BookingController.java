...
public class BookingController {
    ...
    public String bookBus(@PathVariable String busId,
        ...
        if (bus == null) return "redirect:/search";

        List<Integer> seatNumbers = Arrays.stream(seatNumber.split(",")).map(String::trim).map(Integer::parseInt).toList();

        List<Passenger> passengers = new ArrayList<>();
        for (int i = 0; i < passengerName.size(); i++) {
            passengers.add(new Passenger(passengerName.get(i), passengerAge.get(i), passengerGender.get(i), seatNumbers.get(i), seatType.get(i)));
        }
        ...
        Booking booking = Booking.builder()
                ...
                .travelDate(java.sql.Date.valueOf(travelDate))
    ...
    @GetMapping("/history")
    @Operation(summary = "View user booking history")
    ...
    public Map<String, Object> getAllBookedSeats(@PathVariable String busId,
                                                 @RequestParam String travelDate) {
        List<Booking> bookings = bookingRepository.findByBusIdAndTravelDate(busId, java.sql.Date.valueOf(travelDate));
